...
class StoreByBarcodeListTests(APITestCase):
    ...
    def test_get_store_by_barcode(self):
        response = self.client.get(f"{self.url}?barcode=123456")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.data[0]['name'], 'Test Store')

    def test_get_store_by_non_existing_barcode(self):
        response = self.client.get(f"{self.url}?barcode=111111")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
